...
def read_json(task_name):
    with open(f'p3_{task_name}_train.jsonl', 'r') as json_file:
        train_json_list = list(json_file)

    with open(f'p3_{task_name}_validation.jsonl', 'r') as json_file:
        val_json_list = list(json_file)

    train_data_list = []
    for json_str in train_json_list:
        train_data_list.append(list(json.loads(json_str).values()))

    val_data_list = []
    for json_str in val_json_list:
        val_data_list.append(list(json.loads(json_str).values()))
...
def save_translated_json(mydict, task_name):
    # save json translated file test
    json_object = json.dumps(mydict, indent=2, ensure_ascii=False)
    
    # Writing to .jsonl
    with open(f'p3_translated_{task_name}.jsonl', "w", encoding='utf-8') as outfile:
...
def main():
    ...
    for task_name in TASK_LIST:
        try:
            data = read_json(task_name)
            data_chunks = list(divide_chunks(data, 1000))
            print("Starting task...")
            ...
            save_translated_json(translated_list_1, task_name)
            save_translated_json(translated_list_2, task_name)
